...
import { ServiceHealthPanel } from '../../components/catalog/ServiceHealthPanel';
...
export default function ServiceDetailPage() {
  ...
  // ── Phase 20 — Health & Dependency Graph tab state ──────────────────────
  const [activeTab, setActiveTab] = useState<'overview' | 'health' | 'graph'>('overview');
  const [healthData, setHealthData] = useState<ServiceHealthResult | null>(null);
  ...
  const loadHealth = async () => {
    ...
    try {
      const { data } = await catalogService.getServiceHealth(service.id);
      setHealthData(data);
    } catch {
      /* service may not have health data yet */
    } finally {
  ...
  const loadGraph = async () => {
    ...
    try {
      const { data } = await catalogService.getDependencyGraph(service.id);
      setGraphData(data);
    } catch {
      /* graph unavailable */
    } finally {
  ...
  const handleTabClick = (tab: 'overview' | 'health' | 'graph') => {
    setActiveTab(tab);
    if (tab === 'health' && !healthData) loadHealth();
      ...
      <main className="max-w-5xl mx-auto p-8">
        ...
        <div className="flex gap-1 border-b border-gray-200 mb-6">
          {[
            ...
            { id: 'health' as const,   label: '🏥 Health & Checklist' },
        ...
        {activeTab === 'health' && (
          healthLoading ? (
            ...
            <ServiceHealthPanel
              ...
              healthLevel={healthData.healthLevel}
              activeAlerts={healthData.activeAlerts}
              failureRate={healthData.failureRate}
              onboardingScore={healthData.onboardingScore}
              checklist={healthData.checklist}
            />
          ) : (
            <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
              <p className="text-gray-400 text-sm">
                Health data unavailable — service-catalog-service may be unable to reach dependent services
              </p>
            </div>
          )
        )}
        ...
        {activeTab === 'graph' && (
          graphLoading ? (
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-8 text-center">
              <p className="text-gray-400 text-sm">Dependency graph unavailable</p>
            </div>
          )
        )}
      </main>
