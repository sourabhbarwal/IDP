...
export class LocalAuditPublisher implements AuditPublisher {
  ...
  async publish(event: AuditEvent): Promise<void> {
    ...
    await this.repo.save(this.repo.create({ userId: event.userId, action: event.action, resourceType: event.resourceType, resourceId: event.resourceId, result: event.result, ipAddress: event.ipAddress, metadata: event.metadata ?? null }));
  }
}